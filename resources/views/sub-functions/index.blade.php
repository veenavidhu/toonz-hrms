<x-app-layout>
    <x-slot name="header">Sub Functions</x-slot>
    ...
    <div class="px-6 pb-12 max-w-[1600px] mx-auto">
        ...
        <div class="glass-card overflow-hidden bg-white">
            ...
            <div class="p-6 md:p-8 border-b border-gray-100/50 bg-white/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl font-black text-gray-800 tracking-tight">Sub Functions</h2>
                    <p class="text-sm text-gray-500 font-medium mt-1">Manage corporate sub functions and their heads.</p>
                </div>
                <div class="flex items-center gap-3 flex-wrap">

                    {{-- Download Template --}}
                    <a href="{{ route('sub-functions.download-template') }}"
                    ...
                    {{-- Add New --}}
                    <a href="{{ route('sub-functions.create') }}"
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/30">
                <form method="GET" action="{{ route('sub-functions.index') }}" class="flex items-center gap-3">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100 w-48">Code</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">Name</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">Head</th>
                    ...
                    <tbody class="divide-y divide-gray-100/50 text-sm">
                        @if (count($subFunctions) > 0)
                            @foreach ($subFunctions as $subFunction)
                            <tr class="hover:bg-blue-50/40 transition-colors group">
                                <td class="px-6 py-4 font-bold text-gray-400 text-center">{{ $loop->iteration + ($subFunctions->currentPage() - 1) * $subFunctions->perPage() }}</td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg bg-blue-50 text-[#004499] text-xs font-black tracking-wide uppercase">
                                        {{ $subFunction->sub_function_code ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 font-bold text-[#004499]">{{ $subFunction->sub_function_name }}</td>
                                <td class="px-6 py-4 text-gray-500 font-medium">{{ $subFunction->head->name ?? '—' }}</td>
                                <td class="px-6 py-4">
                                    @if ($subFunction->status)
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('sub-functions.edit', $subFunction) }}"
                                        ...
                                        <form action="{{ route('sub-functions.destroy', $subFunction) }}" method="POST" class="inline"
                                            onsubmit="return confirm('Delete this sub function?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @else
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-sm font-bold text-gray-500">No sub functions found</p>
                                    <p class="text-xs text-gray-400 mt-1">Get started by creating your first sub function.</p>
                                    <a href="{{ route('sub-functions.create') }}"
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 class="text-sm font-black text-gray-800 uppercase tracking-widest">Import Sub Functions</h3>
            ...
            <form action="{{ route('sub-functions.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                <div class="space-y-4">
