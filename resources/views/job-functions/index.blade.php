    ...
    <div class="px-6 pb-12 max-w-[1600px] mx-auto">
        ...
        <div class="glass-card overflow-hidden bg-white">
            ...
            <div class="p-6 md:p-8 border-b border-gray-100/50 bg-white/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl font-black text-gray-800 tracking-tight">Functions</h2>
                    <p class="text-sm text-gray-500 font-medium mt-1">Manage corporate functions like IT, Accounts, HR, Operations, etc.</p>
                </div>
                <div class="flex items-center gap-3 flex-wrap">

                    {{-- Download Template --}}
                    <a href="{{ route('job-functions.download-template') }}"
                    ...
                    {{-- Add New --}}
                    <a href="{{ route('job-functions.create') }}"
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/30">
                <form method="GET" action="{{ route('job-functions.index') }}" class="flex items-center gap-3">
                    <div class="relative flex-1 max-w-sm">
                        ...
                        <input type="text" name="search" value="{{ $search }}"
                            placeholder="Search by function name or code..."
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100 w-48">Function Code</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">Function Name</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">Description</th>
                    ...
                    <tbody class="divide-y divide-gray-100/50 text-sm">
                        @if (count($jobFunctions) > 0)
                            ...
                            <tr class="hover:bg-blue-50/40 transition-colors group">
                                <td class="px-6 py-4 font-bold text-gray-400 text-center">{{ $loop->iteration + ($jobFunctions->currentPage() - 1) * $jobFunctions->perPage() }}</td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg bg-blue-50 text-[#004499] text-xs font-black tracking-wide uppercase">
                                        {{ $function->function_code ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 font-bold text-[#004499]">{{ $function->function_name }}</td>
                                <td class="px-6 py-4 text-gray-500 font-medium">{{ $function->description ?? '—' }}</td>
                                <td class="px-6 py-4">
                                    @if ($function->status)
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('job-functions.edit', $function) }}"
                                        ...
                                        <form action="{{ route('job-functions.destroy', $function) }}" method="POST" class="inline"
                                            onsubmit="return confirm('Delete this function?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @else
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-sm font-bold text-gray-500">No functions found</p>
                                    <p class="text-xs text-gray-400 mt-1">Get started by creating your first corporate function.</p>
                                    <a href="{{ route('job-functions.create') }}"
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 class="text-sm font-black text-gray-800 uppercase tracking-widest">Import Functions</h3>
            ...
            <form action="{{ route('job-functions.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                <div class="space-y-4">
                    ...
                    <div class="bg-blue-50 text-blue-800 text-xs p-3 rounded-xl flex items-start gap-2">
                        <svg class="w-4 h-4 text-[#004499] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <p>Format: <strong>Function Name, Function Code, Description</strong>. Download the blank template for reference.</p>
                    </div>
                </div>
