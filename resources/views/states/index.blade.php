<x-app-layout>
    ...
    <div class="px-6 pb-12 max-w-[1600px] mx-auto">
        <div class="glass-card overflow-hidden bg-white">
            <div class="p-6 md:p-8 border-b border-gray-100/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl font-black text-gray-800 tracking-tight">States List</h2>
                </div>
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/30">
                <form method="GET" action="{{ route('states.index') }}" class="flex items-center gap-3">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search by state name..."
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">State Name</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">Status</th>
                    ...
                    <tbody class="divide-y divide-gray-100/50 text-sm">
                        ...
                        <tr class="hover:bg-blue-50/40 transition-colors group">
                            <td class="px-6 py-4 font-bold text-gray-400 text-center">{{ $loop->iteration + ($states->currentPage() - 1) * $states->perPage() }}</td>
                            <td class="px-6 py-4 font-bold text-gray-600">{{ $state->country->name }}</td>
                            <td class="px-6 py-4 font-bold text-[#004499]">{{ $state->name }}</td>
                            <td class="px-6 py-4">
                                @if ($state->status)
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('states.edit', $state) }}" class="text-gray-400 hover:text-[#004499] mx-2">Edit</a>
                                <form action="{{ route('states.destroy', $state) }}" method="POST" class="inline">
                                    @csrf @method('DELETE')
            ...
            <div class="px-6 py-4">{{ $states->links() }}</div>
        </div>
    </div>
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 class="text-sm font-black text-gray-800 uppercase tracking-widest mb-4">Import States</h3>
