<x-app-layout>
    <x-slot name="header">Edit Dynamic Role</x-slot>
        ...
        <div class="glass-card overflow-hidden">
            <div class="px-10 py-8 border-b border-gray-100 bg-white/50 backdrop-blur-md">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 shadow-inner">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                    </div>
                    <div>
                        <h2 class="text-xl font-black text-gray-800 tracking-tight uppercase">UPDATE DYNAMIC ROLE</h2>
                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5 ml-0.5">Edit system dynamic role configuration</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('dynamic-roles.update', $dynamicRole) }}" method="POST" class="p-10 space-y-10 bg-white/30 backdrop-blur-xl">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <input type="text" name="name" required value="{{ old('name', $dynamicRole->name) }}" placeholder="Enter Role Name"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            <input type="date" name="effective_date" required value="{{ old('effective_date', $dynamicRole->effective_date) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="status" class="w-full px-5 py-4 bg-gray-50 border-2 border-gray-100/50 rounded-2xl text-[13px] font-bold text-gray-800 outline-none focus:bg-white focus:border-black focus:ring-4 focus:ring-black/5 transition-all duration-300 shadow-sm border-shadow group-hover:bg-white group-hover:border-gray-200 appearance-none cursor-pointer">
                            <option value="1" {{ old('status', $dynamicRole->status) ? 'selected' : '' }}>ACTIVE</option>
                            <option value="0" {{ !old('status', $dynamicRole->status) ? 'selected' : '' }}>INACTIVE</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="pt-8 border-t border-gray-100 flex flex-col md:flex-row items-center justify-start gap-4">
                    <button type="submit" 
                        class="px-12 py-4 bg-black text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] hover:bg-gray-800 transition-all shadow-xl shadow-black/10 active:scale-95 group flex items-center gap-3">
                        Update Role
