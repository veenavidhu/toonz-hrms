    ...
    <div class="px-6 pb-12 max-w-[1600px] mx-auto">
        <div class="glass-card overflow-hidden bg-white">
            ...
            <div class="p-6 md:p-8 border-b border-gray-100/50 bg-white/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl font-black text-gray-800 tracking-tight">Holidays List</h2>
                    <p class="text-sm text-gray-500 font-medium mt-1">Manage corporate holiday calendar.</p>
                </div>
                <div class="flex items-center gap-3 flex-wrap">

                    {{-- Download Template --}}
                    <a href="{{ route('holidays.download-template') }}"
            ...
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/30">
                <form method="GET" action="{{ route('holidays.index') }}" class="flex items-center gap-3">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100 w-48">Date</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100">Holiday Name</th>
                            <th class="px-6 py-4 text-xs font-black text-gray-500 uppercase tracking-widest border-b border-gray-100 text-center w-32">Type</th>
                    ...
                    <tbody class="divide-y divide-gray-100/50 text-sm">
                        @if (count($holidays) > 0)
                            ...
                            <tr class="hover:bg-blue-50/40 transition-colors group">
                                <td class="px-6 py-4 font-bold text-gray-400 text-center">{{ $loop->iteration + ($holidays->currentPage() - 1) * $holidays->perPage() }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 font-bold text-[#004499]">{{ $holiday->holiday_name }}</td>
                                <td class="px-6 py-4 text-center">
                                    @if ($holiday->is_mandatory)
                                        <span class="text-[10px] font-black uppercase tracking-wider text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-100">Mandatory</span>
                                    @else
                                        <span class="text-[10px] font-black uppercase tracking-wider text-gray-400 bg-gray-50 px-2 py-1 rounded border border-gray-100">Optional</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if ($holiday->status)
                                ...
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-50 mb-4">
                                        <svg class="w-8 h-8 text-[#004499]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 200-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                        </svg>
                                    </div>
                                    <p class="text-sm font-bold text-gray-500">No holidays found</p>
                                    <p class="text-xs text-gray-400 mt-1">Get started by creating your first corporate holiday.</p>
                                    <a href="{{ route('holidays.create') }}"
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 class="text-sm font-black text-gray-800 uppercase tracking-widest">Import Holidays</h3>
            ...
            <form action="{{ route('holidays.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                <div class="space-y-4">
