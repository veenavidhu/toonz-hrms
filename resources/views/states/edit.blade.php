    ...
    <div class="max-w-[1600px] mx-auto px-6 pb-12">
        <div class="glass-card overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100 bg-white/50">
                <h2 class="text-lg font-black text-gray-800 tracking-tight uppercase">UPDATE STATE</h2>
            </div>
            <form action="{{ route('states.update', $state) }}" method="POST" class="p-8 space-y-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $state->name) }}" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-[#004499]/20 focus:border-[#004499] transition-all" required>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <input type="checkbox" name="status" value="1" class="sr-only peer" {{ $state->status ? 'checked' : '' }}>
